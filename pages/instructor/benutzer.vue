<template>
  <section class="pt-10">
    <section class="pb-10 px-10 flex items-center justify-between">
      <div>
        <p class="text-[18px]">Alle ({{ benutzer.length }})</p>
      </div>
...
<script setup>
import mockData from "../../data/benutzerSample.json";

const benutzer = ref(mockData);
...
const displayedBenutzer = computed(() => {
  const startIndex = (currentPage.value - 1) * perPage;
  const endIndex = startIndex + perPage;
  return benutzer.value.slice(startIndex, endIndex);
});

// Calculate the total number of pages
const pageCount = computed(() => Math.ceil(benutzer.value.length / perPage));
