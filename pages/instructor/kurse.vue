<template>
  <section class="pt-10">
    <section class="pb-10 px-10 flex items-center justify-between">
      <div>
        <p class="text-[18px]">Alle ({{ kurse.length }})</p>
      </div>
...
<script setup>
import mockData from "../../data/kurseSample.json";

const kurse = ref(mockData);
...
const displayedKurse = computed(() => {
  const startIndex = (currentPage.value - 1) * perPage;
  const endIndex = startIndex + perPage;
  return kurse.value.slice(startIndex, endIndex);
});

// Calculate the total number of pages
const pageCount = computed(() => Math.ceil(kurse.value.length / perPage));
